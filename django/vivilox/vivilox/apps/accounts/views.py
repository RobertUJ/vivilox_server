...
def profile_user(request):
    ...
    if request.user.profile.user_type == "1":
        ...
        _feedback =  feedbacks.objects.filter(client=request.user).order_by('-id')
        ...
        _feedback =  feedbacks.objects.filter(artist=request.user).order_by('-id')
...
def my_store(request):
	_store = item.objects.filter(user=request.user).order_by("-id")
...
def show_proposal_artist(request):
	...
	myContest = contest.objects.filter(id__in=_contest).order_by('-date_start')
...
def edit_profile(request):
	if request.method == "POST":
		...
		if uform.is_valid() and pform.is_valid():
			user = uform.save()
			pform.save()
			ctx = {'frmUser':uform,'frmProfile':pform,'msg':"Congrats, information stored successfully.",'redirect':1}
...
def register_user(request):
	...
	if request.method == "POST":
		...
		if objForm.is_valid():
			...
			country_ 			  = objForm.cleaned_data["country"]
			province_ 			  = objForm.cleaned_data["province"]
			new_profile           = UserProfile.objects.get(user=new_user)
			new_profile.user_type = usertype_
			new_profile.country   = country_
			new_profile.province  = province_

			new_profile.save()

			''' Send Email '''
			subject = "Welcome to Vivilox  %s %s" %(new_user.first_name,new_user.last_name)
			to_admin = 'roberto@newemage.com'
			html_content = "Hello!, your account %s has been created. Congratulations!!" %(new_user.username)
			msg = EmailMultiAlternatives(subject,html_content,'from@server.com',[new_user.email])
			msg.attach_alternative(html_content,'text/html') #Definimos el contenido como HTML
			msg.send() #enviamos el correo

			ctx = {'title':'Welcome to Vivilox','message':'<h4>Welcome to vivilox</h4><p>To enter the site click on <a href="/accounts/login/">login</a> </p>'}
