...
def view_feedback(request):
	...
	if request.method == "POST":
		...
		if idContest == 0:
			if user_type == "1":
				_feedback = feedback.objects.filter(client=_user)
			elif user_type == "2":
				_feedback = feedback.objects.filter(artist=_user)
			elif user_type == "3":
			...
			if user_type == "1":
				_feedback = feedback.objects.filter(client=_user,contest=_contest)
			elif user_type == "2":
				_feedback = feedback.objects.filter(artist=_user,contest=_contest)
			elif user_type == "3":
				return HttpResponseRedirect('/admin/')
			else:
				return HttpResponseRedirect('/')
	...
	if request.method == "POST":
	    frmFeedback = formFeedBack(request.POST)
            if frmFeedback.is_valid():
            	...
            	frmNewUncommit.save()
            	ctx = {'frmFeedBack':frmFeedback,'message':'Feedback saved correctly, thanks for your comments.'}
...
def feedback_delete(request,idFeedback):
	...
	instanceFeedback = get_object_or_404(feedback,id=_idFeedback)
